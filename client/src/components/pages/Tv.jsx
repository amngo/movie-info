    ...
    <div className='flex flex-col justify-between w-full h-full'>
      <div className='flex flex-col h-full'>
        <div
          ...
          style={{
            background: `linear-gradient(180deg, rgba(0,0,0,0.50) 60%, rgba(0,0,0,0.95) 85%, rgba(0,0,0,1) 100%), url(${tv.backdrops[0] &&
        ...
        >
          <div className='flex'>
            <div className='flex flex-col items-center h-auto rounded-lg'>
              <img src={tv.poster_path ? `https://image.tmdb.org/t/p/w500${tv.poster_path}` : tvPlaceholder} alt='' style={{ minWidth: '16rem', maxWidth: '16rem' }} />
              <button className='inline-block px-4 py-2 mt-4 text-lg font-bold border rounded cursor-pointer bg-movie-blue border-movie-blue'>Play Trailer</button>
            </div>
                ...
                <div>
                  ...
                  {tv.genres.length !== 0 && (
                    <div className='mb-2 text-sm'>
                      {tv.genres.map(genre => (
                        <Label key={genre.name} name={genre.name} classes='m-1' />
                      ))}
                    </div>
                  )}
                  <div className=''>
                    ...
                    <div>
                      ...
                      <div className='text-sm'>
                        {tv.keywords.map(keyword => (
                          <Label key={keyword.name} name={keyword.name} classes='m-1' />
                        ))}
                      </div>
                    </div>
                  )}
      ...
      <div className='px-8 py-8'>
        ...
        <div className='w-full mb-8'>
          {tv.similarMovies.total_results !== 0 && (
            <div className='flex flex-col'>
              <h2 className='mb-4 text-xl'>Similar Shows ({tv.similarMovies.total_results})</h2>
              <MediaList media={tv.similarMovies.results.slice(0, 5)} mediaType='tv' />
            </div>
          )}
        </div>
      </div>
